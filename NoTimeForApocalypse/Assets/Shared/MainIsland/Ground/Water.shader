...
{  
    Properties
    {
        ...
        _DeepColor ("Deep Water", Color) = (0, 0, 1, 1)
        _CoastColor ("Flat Water", Color) = (1, 1, 1, 1)
    }
    SubShader
    {
		ZWrite Off
        ...
        Pass
        {
            ...
            float4 frag(Fragment IN) : COLOR
            {
                ...
                float4 deepCol = lerp(_DeepColor, _CoastColor, 
                        saturate(
                            sin(
                                (_Time.y *2 + cnoise(IN.uv_MainTex * 50 + _Time.y * 0.05)*10) * 0.25
                            ) * 0.1 + 0.1
                        )
                        );
                half4 o = lerp(deepCol, _CoastColor, value);
